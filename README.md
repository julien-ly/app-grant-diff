...
The tool therefore uses `$expand` to establish which principals hold grants, then re-reads individually every non-empty expanded collection, and issues a targeted read wherever a zero carries a conclusion. On a tenant of 305 principals that is four pages plus nine individual reads, against 305 in a naive enumeration. A difference between the two reads is reported as `ExpandCollectionDiffers` and surfaced in `observation.expandCompleteness`.
...
`ExpandCollectionDiffers` the expanded collection and the individual re-read do not contain the same assignments. The code names the difference, not its cause. At the documented cap, a capped expansion and an assignment granted between the two reads produce exactly the same shape, and two reads cannot separate them. Even calling it incomplete would assert more than is known: a grant that arrived after the expansion leaves that expansion complete at the moment it was taken. `details.consistentWithDocumentedCap` says whether the cap could explain the difference, `details.causeEstablished` is always false, and `details.onlyInReRead` names the permissions rather than counting them, because how many differ does not tell a reader whether they were read-only roles or `Directory.ReadWrite.All`. The re-read is retained as the later observation.
