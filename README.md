...
`ExpandCollectionTruncated` an expanded collection was incomplete and was repaired by an individual read. Raised only when the expansion returned at least the documented cap and nothing was lost between the reads: an expansion cannot have been capped below the cap, so a smaller difference is reported as a disagreement instead. `details` names the permissions `$expand` omitted, because how many were missing does not tell a reader whether they were read-only roles or `Directory.ReadWrite.All`, and links the page where the cap is published.
`ExpandReadsDisagree` the two reads differ and the cause is not established. Either something present in the expansion is absent from the re-read, or the difference is smaller than the documented cap could explain. A change between the two reads produces the same shape, so `details.causeEstablished` is false. The re-read is retained as the later observation.
